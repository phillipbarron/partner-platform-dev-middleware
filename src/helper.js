...
const convertTokenToCookie = (partnerPlatformHostTokens) =>
  Object.entries(partnerPlatformHostTokens).map(([hostname, tokens]) => ({
    hostname,
    idToken: {
      name: "ckns_pp_id",
...
exports.generateLoginCookies = async () => {
  const partnerPlatformToken = await fetchToken(
    { hostname: `https://optimo.${process.env.ENV}.tools.bbc.co.uk` },
    {
